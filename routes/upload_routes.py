...
def upload_file():
    ...
    for i, gen in enumerate(ga_result_data["generations_stats"]):
        ...
        prog = ga_result_data["progress_data"][i]
        new_prog_entry = GAProgression(
            generation_id=new_gen_entry.id,
            best_overall_fitness=prog["best_overall"],
            generation_best_fitness=prog["generation_best"]
        )
        db_session.add(new_prog_entry)
        db_session.commit()
    ...
    return jsonify({
        "message": "File uploaded and both algorithms executed successfully.",
        "ga_result_id": new_ga_entry.id,
